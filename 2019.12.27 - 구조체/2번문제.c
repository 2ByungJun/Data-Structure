...
//   설  명     : data.txt파일에 p64의 14번 문제와 같이 n이 입력되어
...
int data_file(unsigned int n) // 1+2+3+ ... + n 값을 계산하는 재귀함수
{
	if ( n > 0 )
	{
		return n + data_file(n-1); // n > 0 경우 n값과 data_file함수를 n-1 상태로 리턴
	}
	if ( n == 0 )
...
int main()
{
	clock_t start=0,finish=0;  // 시간을 재는 시작과 마침의 변수 선언
	double duration;           // 시간차이를 저장하는 변수    
	unsigned int n;           // 파일 데이터 값을 저장할 변수
	int i;           // 반복 증가값 변수
	int sum;         // 합을 누적시키는 변수

	FILE *fp = fopen("data2.txt","r"); // data2.txt 파일을 읽기형식으로 오픈
	...
	while(!feof(fp))   // 파일을 끝까지 읽어주는 반복문
	{
		...
		fscanf(fp,"%d",&n);  // 파일 데이터 값을 n값에 대입한다 
